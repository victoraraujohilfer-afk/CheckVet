...
let PDFService = class PDFService {
    ...
    async generateConsultationPDF(consultation) {
    ...
    renderHeader(doc, data) {
        ...
        if (data.veterinarian.clinicLogoUrl) {
            try {
                if (data.veterinarian.clinicLogoUrl.startsWith('http://') ||
        ...
        doc
            ...
            .text(`Data: ${this.formatDate(data.date)} às ${this.formatTime(data.date)}`, rightX, 50, { width: 145, align: 'right' });
        doc
            .fontSize(8)
    ...
    renderPatientInfo(doc, data) {
        ...
        this.renderInfoField(doc, 'Peso', data.patient.weight ? `${data.patient.weight} kg` : '—', col2X, doc.y);
        doc.moveDown(2);
    }
    renderConsultationInfo(doc, data) {
        ...
        data.checklist.forEach((item, index) => {
            ...
            if (item.completed && item.completedAt) {
                doc
                    ...
                    .fontSize(8)
                    .fillColor(this.COLORS.success)
                    .text(`✓ ${this.formatDate(item.completedAt)} ${this.formatTime(item.completedAt)}`, statusX, itemY, { width: 80, align: 'right' });
            }
            else {
                doc
                    .font(this.FONTS.normal)
    ...
    renderProcedures(doc, procedures) {
        if (!procedures)
            return;
        ...
        procedures.forEach((proc) => {
            doc
                ...
                .text(proc.value != null ? this.formatCurrency(proc.value) : '—', valueX, currentY, { width: 90, align: 'right' });
            if (proc.value) {
                total += proc.value;
            }
            currentY += 20;
    ...
    renderSignatureFields(doc, data) {
        if (doc.y > 650) {
            doc.addPage();
        }
        doc.moveDown(2);
    ...
    renderFooter(doc) {
        ...
        doc
            ...
            .fontSize(8)
            .fillColor(this.COLORS.lightText)
            .text('Este documento foi gerado eletronicamente', 50, 770, { width: 250, align: 'left' });
        doc
            .fontSize(7)
        ...
        doc
            ...
            .fontSize(7)
            .fillColor(this.COLORS.lightText)
            .text(`Gerado em: ${this.formatDate(new Date())}`, 300, 782, { width: 240, align: 'right' });
    }
    ...
    renderInfoField(doc, label, value, x, y, width = 135) {
        doc
            .font(this.FONTS.bold)
            .fontSize(8)
            .fillColor(this.COLORS.lightText)
    ...
    translateConsultationType(type) {
        const translations = {
